...
실행 순서:
  ...
  STEP 4: 기관 수요예측 경쟁률 보강 (Gemini Flash 2.0 + Google Search)
          조건: Notion DB에 상장일자 존재 & 경쟁률 미수집인 종목만
  STEP 5: 실행 결과 저장
"""
...
def main():
    ...
    # ── STEP 4: 기관 수요예측 경쟁률 보강 ───────────────────
    #   대상: Notion DB에서 상장일자 有 & 경쟁률 無 인 종목
    #   방법: Gemini Flash 2.0 + Google Search Grounding
    logger.info("\n[STEP 4] 기관 수요예측 경쟁률 보강 (Gemini Flash 2.0)")
    logger.info("대상 조건: Notion DB 상장일자 존재 AND 경쟁률 미수집")
    try:
        pending = notion_client.query_pages_with_listing_date()
        logger.info(f"보강 대상: {len(pending)}건")

        for page in pending:
            name     = page.get("종목명", "")
            page_id  = page.get("id", "")
            listing  = page.get("상장일자", "")

            if not name or not page_id:
                continue

            logger.info(f"[Gemini 조회] {name} (상장일자: {listing})")
            rate = gemini_fetcher.fetch_competition_rate(name)

            if rate is not None:
                ok = notion_client.update_competition_rate(page_id, rate)
                if ok:
                    stats["competition_updated"] += 1
                    logger.info(f"  → 경쟁률 갱신 완료: {rate}:1")
                else:
                    logger.warning(f"  → Notion 업데이트 실패: {name}")
                    stats["errors"].append(f"STEP4-update-{name}")
            else:
                stats["competition_skipped"] += 1
                logger.info(f"  → 경쟁률 미확인 (수요예측 정보 없음): {name}")
    ...
    logger.info(f"  경쟁률 미확인:    {stats['competition_skipped']}건")
    if stats["errors"]:
